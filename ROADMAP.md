...
| 13  | License             | **Apache-2.0**                                                                    | Open-vs-commercial IS the positioning (§2), so BSL/fair-source is out. Not copyleft: the host SDK links into proprietary RN apps — GPL/AGPL would block the target adopter. Over MIT for the explicit patent grant + retaliation clause: enterprises embedding a runtime clear Apache-2.0 faster, and it matches the norm for embedded infra (Kubernetes, Swift, Android) |
